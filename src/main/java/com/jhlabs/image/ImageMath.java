...
//		return (float)Math.pow(a, Math.log(b) / Math.log(0.5));
    ...
    public static int bilinearInterpolate(float x, float y, int nw, int ne, int sw, int se) {
        float cx = 1.0f - x;
        float cy = 1.0f - y;
        int a = (int) (cy * (cx * ((nw >> 24) & 0xff) + x * ((ne >> 24) & 0xff)) + y * (cx * ((sw >> 24) & 0xff) + x * ((se >> 24) & 0xff)));
        int r = (int) (cy * (cx * (((nw >> 16) & 0xff)) + x * ((ne >> 16) & 0xff)) + y * (cx * ((sw >> 16) & 0xff) + x * ((se >> 16) & 0xff)));
        int g = (int) (cy * (cx * ((nw >> 8) & 0xff) + x * ((ne >> 8) & 0xff)) + y * (cx * ((sw >> 8) & 0xff) + x * ((se >> 8) & 0xff)));
        int b = (int) (cy * (cx * (nw & 0xff) + x * (ne & 0xff)) + y * (cx * (sw & 0xff) + x * (se & 0xff)));
     ...
     */
    // Unsigned byte
    public static byte bilinearInterpolateGray(float x, float y, byte nw, byte ne, byte sw, byte se) {
        float cx = 1.0f - x;
        float cy = 1.0f - y;
        return (byte) ((((int) nw & 0xff) * cx + ((int) ne & 0xff) * x) * cy + (((int) sw & 0xff) * cx + ((int) se & 0xff) * x) * y);
    }
